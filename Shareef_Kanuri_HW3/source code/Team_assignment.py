...
input_text = pd.read_csv('datasets/team/input_data.dat', header=None)
...
svd = TruncatedSVD(n_components=41, n_iter=7, random_state=42)
svd.fit(X)
...
# scaled_data = scaler.fit_transform(X_dense)
#
...
#
# scaled_data.shape
# scaled_input = pd.DataFrame(scaled_data)
# scaled_input.head()
...
def get_distances(centroids, features):
    ...
    for each_centroid in centroids:
        # distances.append(l2_norm(features - centroids[each_centroid]))
        # print(features)
        # print('-' * 20)
        # print(features.reshape(-1, 500))
...
for centroid in centroids:
    ...
    # plt.scatter(clf.centroids[centroid], clf.centroids[centroid], marker="o", color="no_of_components", s=150, linewidths=5)
